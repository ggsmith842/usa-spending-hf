...
def convert_to_parquet(
    ...
    print(f"[transform] Starting convert_to_parquet bucket={bucket_id} source={remote_zip_path}")
    with TemporaryDirectory(dir=DATA_DIR) as tmp:
        ...
        with zipfile.ZipFile(zip_path) as zf:
            ...
            print(f"[transform] Syncing {parquet_count} parquet file(s) to hf://buckets/{bucket_id}/parquet/")
            sync_bucket(
                str(out_dir),
